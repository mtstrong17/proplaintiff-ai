    ...
    insights: [
      {
        ...
        finding: "Initial trauma assessment confirms severe injuries consistent with claimed accident mechanism",
        significance: "high",
...
type LegalInsight = {
  ...
  | { cases: string[]; details?: never }
  | { details: string; cases?: never }
);
    ...
    insights: [
      {
        ...
        description: "Three recent cases in the same jurisdiction with similar fact patterns resulted in favorable verdicts",
        relevance: "high",
              ...
              {documentInsights.map((section) => (
                <Card key={section.title}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      AI-analyzed insights from {section.type === 'document' ? 'documents' : 'testimonies'}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <div key={index} className="rounded-lg bg-muted/50 p-4">
                          <div className="flex items-start justify-between">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              insight.significance === 'high' ? 'bg-green-100 text-green-700' :
                              'bg-blue-100 text-blue-700'
                            }`}>
                              {insight.significance.charAt(0).toUpperCase() + insight.significance.slice(1)} Significance
                            </span>
                          </div>
                      ...
                      {section.insights.map((insight, index) => (
                        <div key={index} className="rounded-lg bg-muted/50 p-4">
                          <div className="flex items-start justify-between">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              insight.relevance === 'high' ? 'bg-green-100 text-green-700' :
                              'bg-blue-100 text-blue-700'
                            }`}>
                          ...
                          {insight.cases && (
                            <div className="mt-2 flex flex-wrap gap-2">
                              {insight.cases.map((caseName, caseIndex) => (
                                <span key={caseIndex} className="text-xs bg-muted-foreground/10 px-2 py-1 rounded">
                                  {caseName}
                                </span>
                              ))}
                            </div>
          ...
          <TabsContent value="timeline">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Key events and developments in chronological order</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                    ...
                    <div key={index} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`rounded-full p-2 ${
                          event.type === 'key' ? 'bg-primary text-primary-foreground' :
                          event.type === 'medical' ? 'bg-blue-100 text-blue-700' :
                          event.type === 'legal' ? 'bg-purple-100 text-purple-700' :
                          'bg-green-100 text-green-700'
                        }`}>
                          {event.type === 'key' ? <AlertTriangle className="h-4 w-4" /> :
                           event.type === 'medical' ? <FileText className="h-4 w-4" /> :
                           event.type === 'legal' ? <Scale className="h-4 w-4" /> :
